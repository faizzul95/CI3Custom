...
class MY_DB_mysqli_driver extends CI_DB_mysqli_driver
{
    ...
    */
    ...
    public function __construct($params)
    {
        ...
        $this->CI->config->load('customs/trail');
    }
    ...
    public function insert($table = '', $set = NULL, $escape = NULL)
    {
        ...
        $lastID = parent::insert_id();

        $this->trail($status, 'insert', $table, $set);

        return ['status' => $status, 'id' => $lastID];
    }
    ...
    public function trail($status, $event, $table, $set = NULL, $previous_values = NULL)
    {
        ...
        if ($this->CI->db->table_exists($this->CI->config->item('audit_table_name'))) {
            Logs(
                $event,
                [
                    'old_value' => $old_value,
                ...
                $table,
                $this->CI->uri->ruri_string()
            );

            return parent::insert(
                $this->CI->config->item('audit_table_name'),
                [
                    'user_id' => (isLoginCheck()) ? currentUserID() : 0,
                    'role_id' => (isLoginCheck()) ? currentRoleID() : 0,
                    'user_fullname' => (isLoginCheck()) ? currentUserFullName() : 'guest',
                    'event' => $event,
                    'table_name' => $table,
